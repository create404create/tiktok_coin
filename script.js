/* ===============================
   ...
   Replace the sample bundles below with the EXACT prices from your video.
   Each item: { coins: number, price: number, note?: string }
*/
const COIN_BUNDLES = [
  ...
  { coins: "custom", price: 0 } // leave last for custom
];

/* If you want custom price to auto-calc:
   We’ll compute by average per-coin rate from the defined bundles (excluding custom). */
...
/* ===============================
   State
   =============================== */
let selectedBundle = null; // {coins, price}
...
function createCoinCard(bundle, idx) {
  ...
  div.innerHTML = `
    <div class="coin-top">
      <img class="coin-icon" src="https://upload.wikimedia.org/wikipedia/commons/3/34/TikTok_Icon_Black.svg" alt="Coin" />
...
function handleCoinSelect(idx) {
  ...
  // enable checkout after a selection
...
/* Simple fake validation (demo only) */
...
checkoutForm.addEventListener("submit", (e) => {
  ...
  if (!uOk || !hasSelection || !fieldsOk || !cardOK) {
    formHint.textContent = !cardOK ? "Enter a valid (demo) card number like 4242 4242 4242 4242." : "Please complete all fields.";
    formHint.classList.add("show");
    return;
  }
  formHint.classList.remove("show");

  // Show fake loading → success
...
document.addEventListener("DOMContentLoaded", () => {
  renderCoinGrid();
  updateSummary();
  // helpful placeholders for demo:
  cardNumber.addEventListener("focus", () => {
    if (!cardNumber.value) cardNumber.value = "4242 4242 4242 4242"; // common demo number
  });
});
